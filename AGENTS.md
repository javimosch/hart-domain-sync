...
- `gh issue list --state open` returns `[]`; issues #68, #69, and #70 are all CLOSED and their fixes are already in `origin/master` (`9402e84` / PR #73):
  - `url_has_path_after_scheme` accepts RFC 3986 URI schemes (`h2c://`, `coap+tcp://`, `s3://`, etc.) while still rejecting a bare scheme with no host.
  - `rule_claimed()` matches the exact ``Host(`domain`)`` form produced by the scanner.
  - `PYCLAIM` and `PYMERGE` rule parsers strip unquoted inline YAML comments before comparing rules.
- `gh pr list --state open` returns PRs #72 (`am/am-add074-dkrwfhi3j297-5230f97a`) and #74 (`am/am-add074-dkry7sv8a5u1-c8862d24`). Both branches bundle the same changes already merged via PR #73 and include stale `AGENTS.md` plan sections; both report `mergeStateStatus: DIRTY`. They should be closed as superseded by the current `origin/master`.
- The current branch is at the same commit as `origin/master` (`9402e84`) with a clean worktree. `bash -n hart-domain-sync.sh hart-domain-hook.sh` passes and `shellcheck hart-domain-sync.sh hart-domain-hook.sh` passes in this environment.
- QA runs the verification gate:
  - `bash -n hart-domain-sync.sh hart-domain-hook.sh`
  - `shellcheck hart-domain-sync.sh hart-domain-hook.sh`
  - manual dry-runs in both directory and file modes covering `WILDCARD_DOMAIN`, `WILDCARD_INSTANCE_DOMAIN`, mixed-case hart entries, `--remove`, leading `*.`, and trailing DNS dots
  - regression for the already-landed fixes:
    - RFC 3986 schemes: `HART_URL=h2c://127.0.0.1:8799`, `coap+tcp://127.0.0.1:8799`, and `HTTP://127.0.0.1:8799` are accepted; `http://` (bare scheme) is still rejected.
    - `rule_claimed`: a foreign router with `rule: "Host(`foo.com`)"` is detected and `foo.com` is skipped.
    - inline YAML comments: a foreign router with `rule: "Host(`foo.com`)" # my router` is detected and the domain is not duplicated.
  - regression under a non-C locale (e.g. `LC_ALL=fr_FR.UTF-8`) for the locale-hardened code: `cf_val()` parsing with `export`/CRLF, `slug()` dot-to-dash, Traefik provider detection, `HART_URL`/`SERVICE_URL` trailing-slash and bare scheme handling, hook event lowercasing, `regex_escape()` dot escaping, mixed-case hart domains and wildcard inputs, and generated Traefik `Host()` / `HostRegexp()` rules and Cloudflare DNS targets lowercased.
- If the gate passes, the current branch should be PR'd with a record-keeping commit that closes PRs #72 and #74 as superseded by `origin/master`; no additional dev code change is required.
