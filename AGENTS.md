...
## 2026-08-23 final status and supersede stale PR #142 (am-add074-dkw9ayvqf8e1-306bd31e)

- `gh issue list --state open` returns `[]`; no open GitHub issues remain to fix.
- `gh pr list --state open` returns only PR #142 (`am/am-add074-dkw875oyyc08-274b5b4d`, `docs(agents): 2026-08-23 architect plan and empty issue/PR list`). It is stale and superseded by this final-status; its plan/final-status records are already covered by `origin/master`.
- The current branch `am-add074-dkw9ayvqf8e1-306bd31e` is at `origin/master` (`233c7ae`) with a clean worktree and this docs entry; no source-code changes are required.
...
- `bash -n hart-domain-sync.sh hart-domain-hook.sh` and `shellcheck hart-domain-sync.sh hart-domain-hook.sh` both pass.
- QA to re-run the manual verification gate in directory and file modes covering `WILDCARD_DOMAIN`, `WILDCARD_INSTANCE_DOMAIN`, mixed-case hart entries, fast `--remove`, unquoted `#` in `CF_ENV`, and backslash-escaped backticks in foreign routers.
- If the gate passes, the objective is resolved and PR #142 should be closed as superseded. If QA finds a regression or uncovered edge case, open a focused GitHub issue and produce one small conventional-commit PR.
- Closes #142
