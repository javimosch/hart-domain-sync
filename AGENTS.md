...
- QA runs the verification gate focused on the three new fixes:
  ...
  - regression for #68: place a foreign router for `foo.com` in `DEST`/`SINGLE_FILE` with `rule: "Host(`foo.com`)"`; reconcile with hart returning `foo.com`; confirm `hart-foo-com.yml` / the `hart-foo-com` key is skipped with `skip: foo.com is already routed by ...`.
