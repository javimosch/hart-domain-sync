...
#   add/set  → full background reconcile (idempotent, also prunes any stale mappings).
#   remove   → fast background removal of the single per-domain router; the 5-min timer is
#              the safety net if the hook is unavailable.
exec >/dev/null 2>&1
EVENT="${1:-}"; DOMAIN="${2:-}"
